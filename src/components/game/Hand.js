...
const Hand = ({ isValidating }) => {
  const { playerHand, selectedCards, addLetter, gameStatus, clearNewFlags } = useGame();
  ...
  useEffect(() => {
    ...
    if (newCards.length > 0) {
      ...
      setTimeout(() => {
        setAnimatingCards(new Set());
        clearNewFlags();
      }, (newCards.length * 100) + 500); // Total stagger time + animation duration
    }
  }, [playerHand]);
    ...
    <div className="w-full flex flex-wrap gap-4 justify-center p-4 bg-gray-100 rounded-lg shadow-inner">
      {playerHand.map((card, index) => (
        <Card
          ...
          isNew={card.isNew}
          onClick={() => {
            if (!selectedCards.includes(index) && gameStatus === 'playing' && !isValidating) {
